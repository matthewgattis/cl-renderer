#define MAX_DISTANCE        (128.0f)
#define MIN_DELTA           (1e-4f)
...
float getMap(float3 p, struct surface_feature *surf)
{
    ...
    tp = dot(p - (float3)(0, -6, 0), (float3)(0, 1, 0));
    if (tp < t)
    {
        t = tp;
        surf->obj = 2;
...
float4 drawScene(float2 p, float2 *seed, float time)
{
    ...
    float4 view[4] =
    {
        { 0.9293, 0.00180532, -0.369314, 0 },
        { -0.0873118, 0.972713, -0.214948, 0 },
        { 0.358847, 0.231996, 0.904101, 0 },
        { 0.498999, 0.252403, 1.28284, 1 }
    };

    float3 o = 15.0f * view[3].xyz;
    ...
    const float3 light_vec = normalize((float3)(2.0f, 3.0f, 1.0f));
    ...
    for (int i = 0; i < MAX_REFLECTIONS; i++)
    {
        ...
        *seed = (*seed + (float2)(8436.4156f, 7864.1684f)) * ((float)j + 1.0f);
        ...
        //d = getCosineWeightedSample(n, seed);
        d = getSample(n, seed);

        castRay(o, getConeSample(light_vec, 0.00001f, seed), &surf_shad);
    ...
    while (j > 0)
    {
        ...
        else
        {
            ...
            {
                color = 0.6f * (float3)(1, 1, 1);
            }

            if (surf[j].obj_shad == 0)
            {
                float3 sun_light = 1.0f * (float3)(1.0f, 0.9f, 0.8f);
                float ndotl = max(0.0f, dot(surf[j].n, light_vec));
                comp = color * (comp + sun_light);
            }
            else
            {
                comp = color * comp;
